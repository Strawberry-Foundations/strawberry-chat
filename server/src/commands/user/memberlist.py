...
def memberlist_command(socket: socket.socket, username: str, args: list, send):
    ...
    admins      = "\n           ".join([f"{isOnline(result[0])}{LIGHTRED_EX} {memberListNickname(result[0])} {memberListBadge(result[0])}" for result in raw_admins])
    ...
    bots        = "\n           ".join([f"{isOnline(result[0])}{LIGHTMAGENTA_EX} {memberListNickname(result[0])} {memberListBadge(result[0])}" for result in raw_bots])
    ...
    members     = "\n           ".join([f"{isOnline(result[0])}{LIGHTYELLOW_EX} {memberListNickname(result[0])} {memberListBadge(result[0])}" for result in raw_members])
