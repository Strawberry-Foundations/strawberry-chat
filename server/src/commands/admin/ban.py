...
def ban_command(socket: socket.socket, username: str, args: list, send):
    ...
    if doesUserExist(uname) == False:
        ...
        send(f"{LIGHTGREEN_EX + Colors.BOLD}Banned {uname}{RESET + Colors.RESET}")
...
def unban_command(socket: socket.socket, username: str, args: list, send):
    cmd_db = Database(server_dir + "/users.db", check_same_thread=False)
    ...
    if doesUserExist(uname) == False:
        ...
        send(f"{LIGHTGREEN_EX + Colors.BOLD}Unbanned {uname}{RESET + Colors.RESET}")
