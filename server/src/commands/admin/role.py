...
def role_command(socket: socket.socket, username: str, args: list, send):
    ...
    if cmd == "set":
        try:
            ...
            if role not in ["member", "bot", "admin"]:
                send(f"{RED + Colors.BOLD}Invalid role!{RESET + Colors.RESET}")
                return
        
            cmd_db.execute("UPDATE users SET role = ? WHERE username = ?", (role, uname))
            cmd_db.commit()
            
            send(f"{LIGHTGREEN_EX + Colors.BOLD}Role of {uname} was set to {role}{RESET + Colors.RESET}")
        
        except:
            send(f"{RED + Colors.BOLD}Invalid username and/or role!{RESET + Colors.RESET}")
    ...
    # /role get Command
        ...
        try:
            ...
            send(f"{LIGHTGREEN_EX + Colors.BOLD}Role of {uname}: {MAGENTA}{role}{RESET + Colors.RESET}")
    ...
    # /role color Command
        ...
        try:
            ...
            if color.lower() not in role_colors:
                send(f"{RED + Colors.BOLD}Invalid color!{RESET + Colors.RESET}")
                return
        
            cmd_db.execute("UPDATE users SET role_color = ? WHERE username = ?", (color.lower(), uname))
            cmd_db.commit()
            
            send(f"{LIGHTGREEN_EX + Colors.BOLD}Role Color of {uname} was set to {color}{RESET + Colors.RESET}")
    
        except:
            send(f"{RED + Colors.BOLD}Invalid username and/or color!{RESET + Colors.RESET}")
