...
use libstrawberry::string::contains_whitespace;
...
pub async fn client_register(
    ...
    if i16::try_from(registered_users.len()).unwrap() >= CONFIG.config.max_registered_users
        && CONFIG.config.max_registered_users != -1
    {
        ...
        w_client
            ...
            .shutdown()
            .await
            .unwrap_or_else(|_| LOGGER.error(format!("{S2C_ERROR} (core::register::#31)")));
        return;
    }
    ...
    match action {
        MessageAction::Kick | MessageAction::Hide => {
            w_client
                ...
                .await
                .unwrap_or_else(|_| LOGGER.warning(WRITE_PACKET_FAIL));
    ...
    if ["exit", "register", "login", "sid", "list", "user", "info"].contains(&username.as_str()) {
        ...
        w_client
            ...
            .shutdown()
            .await
            .unwrap_or_else(|_| LOGGER.error(format!("{S2C_ERROR} (core::register)")));
        return;
    }
    ...
    DATABASE
        .create_user(user_id, username, registered_password, role_color)
        .await;
}
